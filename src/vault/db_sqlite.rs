...
impl DbInterface for DbSqlite {
    async fn accounts_populated(&self) -> NetResult<bool> {
        let num_accounts: u64 = sqlx::query("SELECT COUNT(*) FROM accounts")
            .fetch_one(&self.pool).await
            .map_err(|err| {
                warn!("Failed to check for existing accounts: {err}");
    ...
    async fn get_account(&self, account_name: &str) -> NetResult<Option<AccountInfo>> {
        ...
        if account.is_none() && self.volatile {
            ...
            if let Ok(api_token) = self.create_api_token(&new_account.account_id,
                                                         "Autogenerated").await {
                info!("API token for '{account_name}' is {}", api_token.token);
            }
            Ok(Some(new_account))
        } else {
            Ok(account)
